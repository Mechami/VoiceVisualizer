...
function PANEL:Setup(ply)  
	...
	hook.Add("Think", "PanelThink"..ply:UniqueID(), function()
		if IsValid(self) and isfunction(self.UpdatePast) then
			self:UpdatePast()
		end
	end)
...
function PANEL:UpdatePast()
    if self ~= nil and self:Valid() and IsValid(self.ply) then
...
local Red = Vector(1.0, 0.0, 0.0)
local Yellow = Vector(1.0, 1.0, 0.0)
local Green = Vector(0.0, 1.0, 0.0)

function PANEL:GetBarColor(p)
	local p2 = math.Clamp(p, 0.0, 1.0)
	local V1 = LerpVector(p2, Red, Yellow)
	return LerpVector(p2, V1, Green):ToColor()
end
...
function PANEL:VVPaint(w, h)
    ...
    for i,v in pairs(self.Past) do
        ...
        local barcolor = self:GetBarColor(v)
